---
name: visualize
description: "Add a correct, minimal visual to a lesson — a diagram or geometric picture — that renders inline in the Markdown learning log. Use when an idea is genuinely clearer as a picture: a dependency graph, system/flow, sequence, state machine, tree, comparison, or a spatial/geometric thing (coordinate geometry, number line, vectors, a plot, a physical layout). Outsources authoring+rendering to a maker subagent that verifies the image by looking at it, then you embed the returned file."
---

# Visualize

A picture earns its place only when it shows something words can't — shape, structure, direction, relationship, geometry. This skill produces ONE such picture, guarantees it is **correct** (the maker renders it and looks at it before returning), and drops it into the lesson so it renders inline in the Markdown `md-log` file.
...
Put the image directly in your teaching reply using a standard Markdown link relative to the learning directory:

```
![Diagram](viz/viz-<slug>-<timestamp>.png)
```

That's all. The `md-log` extension mirrors your reply text verbatim into the linked `.md`, and visual tools publish into `/home/0xShred/kiroku/learning/viz`, so the relative image link renders from the learning note. LazyVim's Markdown renderer or browser preview can display the image. Introduce the visual in a sentence, then let it carry the idea — don't narrate every element back in prose.
...
- Unique filenames keep learning-note image links unambiguous.

> The makers render through the `visual-tools` extension (Mermaid via a bundled `@mermaid-js/mermaid-cli`; SVG via `rsvg-convert`, fallback ImageMagick). You don't render anything yourself — you only brief the maker and embed the filename it returns.
